# Hardhat 3 + Yarn 4 + Viem 项目搭建指南（基础版）V1  
**基于真实交互界面的精确还原与优化**  
...
| 依赖包名称 | 版本号 | 用途说明 |
| --- | --- | --- |
| `hardhat` | `^3.0.7` | 核心开发框架，提供智能合约编译、测试、部署、脚本执行及内置本地节点（Hardhat Network）等完整开发环境支持 |
| `@nomicfoundation/hardhat-toolbox-viem` | `^5.0.0` | 官方推荐的现代化工具箱，集成 `viem` 作为默认 Ethereum 客户端，同时包含 Solidity 测试、类型生成、Etherscan 验证等常用插件 |
| `@nomicfoundation/hardhat-ignition` | `^3.0.0` | 声明式、可复用的合约部署系统，支持模块化部署配置、依赖管理与部署状态追踪，适用于复杂合约系统的安全部署 |
| `@nomicfoundation/hardhat-ignition-viem` | `^3.0.0` | 为 Ignition 提供 Viem 客户端支持，确保在使用 Ignition 进行部署时可以利用 Viem 的高效和灵活性 |
| `@nomicfoundation/hardhat-keystore` | `^3.0.0` | 提供对 Hardhat 环境中 keystore 文件的支持，允许开发者通过私钥或助记词安全地加载账户 |
| `@nomicfoundation/hardhat-network-helpers` | `^3.0.1` | 提供一系列帮助函数，简化与 Hardhat Network 的交互，例如自动挖掘区块、设置时间等操作 |
| `@nomicfoundation/hardhat-node-test-runner` | `^3.0.0` | 支持运行针对 Node.js 环境的测试，扩展了 Hardhat 的测试能力 |
| `@nomicfoundation/hardhat-viem` | `^3.0.0` | 为 Hardhat 提供 Viem 客户端集成，允许在测试和脚本中使用 Viem 进行以太坊网络交互 |
| `@nomicfoundation/hardhat-viem-assertions` | `^3.0.0` | 基于 Viem 的断言库，方便在测试中进行更精确的状态检查 |
| `@nomicfoundation/hardhat-verify` | `^3.0.4` | 提供智能合约源代码验证功能，支持 Etherscan 等多种区块链浏览器 |
| `typescript` | `~5.8.0` | 支持使用 TypeScript 编写测试脚本、部署脚本和配置文件，提升代码可维护性与类型安全性（固定使用 5.8.x 系列） |
| `@types/node` | `^22.8.5` | 为 Node.js 运行时 API 提供 TypeScript 类型定义，确保在 TS 环境中获得完整的类型提示与校验 |
| `viem` | `^2.30.0` | 高性能、类型安全的 Ethereum TypeScript 客户端库，轻量高效，支持 JSON-RPC 的树摇优化，作为 `ethers.js` 的现代替代方案 |
| `forge-std` | `github:foundry-rs/forge-std#v1.9.4` | Foundry 团队开发的标准 Solidity 库，提供 `Test` 基类、断言函数、日志工具及模糊测试支持，常用于本地测试与断言验证 |

---

### ✅ 版本选择说明

- 所有依赖均基于 **Hardhat v3** 生态进行适配。
- 使用 `~5.8.0` 锁定 TypeScript 小版本，避免破坏性更新。
- `viem` 与 `hardhat-toolbox-viem` 深度集成，推荐在测试与部署中优先使用 viem API。
- `forge-std` 通过 GitHub 安装指定稳定版本 `v1.9.4`，确保 Solidity 测试一致性。
