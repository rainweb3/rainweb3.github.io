...
| 配置字段 | 配置说明 |
|------|------|
| root | 指定该 .editorconfig 文件为项目根配置文件，防止向上级目录搜索其他配置。 |
| charset | 定义文件的字符编码，确保所有编辑器使用统一的编码格式（如 utf-8）保存文件。 |
| indent_style | 指定缩进使用的字符类型，可选值为 `space`（空格）或 `tab`（制表符）。 |
| indent_size | 设置缩进的宽度，即每个缩进级别使用的空格数量。 |
| end_of_line | 定义行尾换行符的格式，如 lf（\n）、crlf（\r\n）或 cr（\r），影响跨平台兼容性。 |
| insert_final_newline | 控制文件末尾是否自动插入一个换行符，确保符合 POSIX 文件标准。 |
| trim_trailing_whitespace | 决定在保存文件时是否自动删除每行末尾的多余空白字符。 |
...
| 配置字段 | 配置说明 |
|------|------|
| PRIVATE_KEY | 用于签署交易的以太坊账户私钥，通常用于部署合约或发送交易，应严格保密。 |
| LOCALHOST_RPC_URL | 指向本地以太坊节点（如Ganache）的JSON-RPC服务地址，用于本地开发和测试。 |
| REMOTE_TEST_PRIVATE_KEY | 用于远程测试网络（如Sepolia）的账户私钥，适用于在测试链上进行部署和交互。 |
| REMOTE_TEST_RPC_URL | 指向远程测试网络的节点服务地址（如通过Alchemy或Infura提供），用于连接以太坊测试链。 |
| ENCRYPTION_PASSWORD | 用于数据加密或解密的密码，常用于保护敏感信息，需保证强度和唯一性。 |


```.env_template
# 以下方式选一种用基本就可以了。
# 方式1：Ganache是一款模拟区块链测试网的本地区块软件
# 本地Ganache私钥地址
# PRIVATE_KEY="你的本地Ganache私钥"
# 本地Ganache配置地址
# LOCALHOST_RPC_URL="你的本地Ganache测试网地址""

# 方式2：
# 1. alchemy-线上免费的测试网（需要科学上网才能访问）
# 私钥
REMOTE_TEST_PRIVATE_KEY="你钱包账户的私钥，如metamask钱包"
# 需要注册账号，登录后，创建你的app应用程序，创建完成后，选择测试网络Sepolia，然后再根据页面提示进行相关操作，生成你的APIKey
REMOTE_TEST_RPC_URL="https://eth-sepolia.g.alchemy.com/v2/你的APIKey"

# 2. 线上另一个公共节点的免费测试网
REMOTE_INFURA_RPC_URL="https://mainnet.infura.io/v3/省略....."
REMOTE_INFURA_PRIVATE_KEY="你钱包账户的私钥，如metamask钱包"

# 加密私钥的密码
# ENCRYPTION_PASSWORD="对私钥进行加密的密码，可自行随意设置值"
```
...
| 配置字段 | 配置说明 |
|------|------|
| env | 指定代码运行的环境，启用对应环境的全局变量和语法支持，如Node.js或ES2021特性。 |
| extends | 继承一个或多个共享的配置（如推荐规则、插件或Prettier集成），实现规则复用和标准化。 |
| parser | 指定用于解析代码的解析器，使ESLint能够理解TypeScript等非原生JavaScript语法。 |
| parserOptions | 配置解析器的选项，如ECMAScript版本、模块类型等，影响语法解析能力。 |
| plugins | 加载插件以提供自定义规则、环境或处理器，扩展ESLint的功能。 |
| rules | 定义具体的代码检查规则及其严重级别（"off"、"warn"、"error"），控制代码风格和质量。 |
| ignorePatterns | 指定应被ESLint忽略的文件或目录模式，避免对生成文件或依赖项进行检查。 |
| overrides | 在特定文件范围内覆盖顶层配置，实现不同文件类型（如.ts和.js）的差异化规则管理。 |
...
| 配置字段 | 配置说明 |
|------|------|
| node_modules | 存放项目依赖的第三方包，通常被忽略以避免版本冲突和仓库膨胀。 |
| .env | 包含环境变量的配置文件，常用于存储敏感信息或运行时配置，应避免提交到版本控制。 |
| commonly_used | 自定义目录，可能用于存放常用脚本、配置或工具，具体用途由项目约定决定。 |
| .vscode | 存放VS Code编辑器的项目级配置（如settings.json、launch.json），影响开发环境设置。 |
| /cache | Hardhat生成的缓存文件目录，用于加速编译和任务执行，可安全删除并重新生成。 |
| /artifacts | Hardhat编译智能合约后生成的产物目录，包含ABI、字节码等文件，供部署和前端调用使用。 |
| /typechain | TypeChain生成的TypeScript类型定义文件目录，用于为智能合约提供强类型支持。 |
| /typechain-types | 另一个TypeChain生成的类型文件目录，可能因配置不同而使用此名称。 |
| /coverage | solidity-coverage工具生成的代码覆盖率报告目录，用于分析测试覆盖情况。 |
| /coverage.json | solidity-coverage生成的覆盖率数据JSON文件，记录合约代码的测试覆盖信息。 |
| ignition/deployments/chain-31337 | Hardhat Ignition模块在本地链（chain ID 31337）上部署合约时生成的部署记录目录。 |
...
| 配置字段 | 配置说明 |
|------|------|
| semi | 控制是否在语句末尾添加分号，设置为 `false` 时省略分号。 |
| singleQuote | 指定是否使用单引号代替双引号表示字符串，设置为 `false` 时使用双引号。 |
| printWidth | 设置代码换行的最大行长度，Prettier会在超过此长度时格式化换行。 |
| tabWidth | 定义缩进的空格数量，对应编辑器中一个缩进层级的宽度。 |
| useTabs | 决定是否使用制表符（Tab）进行缩进，设置为 `false` 时使用空格。 |
| endOfLine | 指定换行符的类型，如 `lf`（\n）、`crlf`（\r\n）等，影响跨平台兼容性。 |
| arrowParens | 设置箭头函数参数括号的使用规则，`avoid` 表示单参数时省略括号。 |
| plugins | 指定加载的Prettier插件，用于支持非JavaScript文件格式（如Solidity）的格式化。 |
...
| 配置字段 | 配置说明 |
|------|------|
| solidity.version | 指定项目使用的Solidity编译器版本，确保合约使用正确的编译器进行构建。 |
| solidity.settings.optimizer.enabled | 启用或禁用Solidity编译器优化器，优化生成的字节码以减少gas消耗。 |
| solidity.settings.optimizer.runs | 设置优化器的模拟运行次数，影响优化强度和部署成本之间的权衡。 |
| networks.sepolia.url | 指定连接Sepolia测试网络的RPC节点地址，用于在测试链上部署和交互合约。 |
| networks.sepolia.accounts | 配置用于Sepolia网络的私钥账户列表，提供签署交易所需的密钥。 |
| networks.localhost.url | 指定本地以太坊节点（如Hardhat Network或Ganache）的RPC服务地址。 |
| etherscan.apiKey | 提供Etherscan API密钥，用于验证和发布合约源代码到Etherscan区块链浏览器。 |
| sourcify.enabled | 控制是否启用Sourcify服务进行合约源代码验证，作为Etherscan的替代方案。 |
...
```package-lock.json
{
    ...
    "packages": {
        ...
        "node_modules/forge-std": {
            "version": "1.9.4",
            "resolved": "git+ssh://git@github.com/foundry-rs/forge-std.git#1eea5bae12ae557d589f9f0f0edae2faa47cb262",
            "dev": true,
...
```package.json
{
    ...
    "scripts": {
        ...
        "compile:sol": "npx ts-node scripts/ts/compile-sol.ts",
        "test": "hardhat test",
        "node": "hardhat node",
        "exec:ts": "node --loader ts-node/esm --no-warnings",
        "deploy:local": "hardhat run scripts/ts/deploy.ts --network localhost",
        "deploy:sepolia": "hardhat run scripts/ts/deploy.ts --network sepolia",
...
| 配置字段 | 配置说明 |
|------|------|
| target | 指定编译生成的JavaScript代码的ECMAScript目标版本，影响代码的兼容性和使用的语法特性。 |
| module | 指定模块代码生成的规范，决定代码如何组织和加载，如CommonJS、ES模块等。 |
| lib | 指定编译过程中包含的库文件，用于提供特定JavaScript运行环境的类型定义。 |
| esModuleInterop | 启用此选项会生成额外的代码以支持从CommonJS模块中正确导入ES模块，改善模块互操作性。 |
| resolveJsonModule | 允许直接导入JSON文件作为模块，TypeScript会根据JSON内容推断其类型。 |
| allowSyntheticDefaultImports | 允许从没有默认导出的模块中进行默认导入，提高模块导入的灵活性。 |
| strict | 启用所有严格的类型检查选项，帮助发现潜在的类型错误，提高代码质量。 |
| skipLibCheck | 跳过对所有声明文件（.d.ts）的类型检查，可以加快编译速度。 |
| forceConsistentCasingInFileNames | 强制文件名的大小写一致性，防止在大小写敏感的文件系统中出现导入错误。 |
| noEmit | 不生成输出文件（如.js、.d.ts等），仅执行类型检查和语法分析。 |
| rootDir | 指定输入文件的根目录，用于确定输出文件的目录结构。 |
| typeRoots | 指定类型声明文件的查找位置，用于覆盖或扩展默认的@types目录查找行为。 |
| include | 指定需要包含在编译过程中的文件或目录模式，明确编译范围。 |
| exclude | 指定需要排除在编译过程之外的文件或目录模式，避免不必要的文件被处理。 |
...
| 配置字段 | 配置说明 |
|------|------|
| editor.tabSize | 设置编辑器中制表符（Tab）显示的空格数量，影响代码缩进的宽度。 |
| editor.detectIndentation | 控制编辑器是否自动检测文件的缩进方式（制表符或空格）和大小，避免格式冲突。 |
| editor.insertSpaces | 决定按下 Tab 键时是否插入空格字符，而非制表符，用于统一缩进风格。 |
| editor.defaultFormatter | 指定用于格式化文档的默认代码格式化工具，影响保存或手动格式化时的行为。 |
| editor.formatOnSave | 启用后，在保存文件时自动格式化代码，保持代码风格一致。 |
| editor.codeActionsOnSave | 定义在保存文件时自动执行的代码操作，例如修复 ESLint 可修复的问题。 |
| files.autoSave | 设置文件的自动保存策略，如在失去焦点时自动保存，减少手动保存操作。 |
| files.eol | 指定文件中使用的行尾结束符，如 LF（\n）或 CRLF（\r\n），影响跨平台兼容性。 |
| typescript.validate.enable | 启用 TypeScript 语言服务在编辑器中的语法和类型错误检查功能。 |
| javascript.validate.enable | 控制是否启用 JavaScript 文件的语法和类型错误检查。 |
| typescript.validate.disableDeepValidation | 禁用深度验证以提升大型项目中 TypeScript 的验证性能。 |
| typescript.suggest.autoImports | 控制是否在代码补全时自动显示来自第三方库的导入建议。 |
| typescript.tsserver.experimental.enableProjectDiagnostics | 启用实验性功能，允许 TypeScript 服务器在后台项目中提供诊断信息。 |
| typescript.preferences.includePackageJsonAutoImports | 控制是否根据 package.json 中的依赖项提供自动导入建议。 |
| typescript.preferences.includeCompletionsForModuleExports | 决定是否在补全列表中包含模块导出的符号。 |
| javascript.preferences.includePackageJsonAutoImports | 控制 JavaScript 文件中是否根据 package.json 依赖提供自动导入建议。 |
